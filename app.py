...
@cache.cached(timeout=3600, key_prefix='bing_image')
def get_bing_image():
...
def recevoir():
    ...
    return render_template("result.html", value=search)

@app.route("/api/search/ddg", methods=["GET"])
def api_ddg():
    search = request.args.get("q")
    results = asyncio.run(scrap_async_ddg(search))
    return jsonify(results)

@app.route("/api/search/bing", methods=["GET"])
def api_bing():
    search = request.args.get("q")
    results = asyncio.run(scrap_async_bing(search))
    return jsonify(results)
